...
function Banner() {
    ...
    function submit() {
        ...
        fetch('https://hou9ygfljg.execute-api.us-east-1.amazonaws.com/Testing/register', {
            ...
            .then(data =>setErrorResp(data.message))
            .catch(error => {
                console.log(error);
            });
    }
    ...
    return (<div>
        <Modal opened={opened} onClose={close} title="Contest Entry" centered>
            ...
            <TextInput
                ...
                withAsterisk
            />
            {errorResp.length === 0 ? <br/> : <p className="text-red-500">{errorResp}</p>}
            <Button onClick={submit} id={"enter-contest"} className="bg-orange-700 hover:bg-amber-300 hover:text-gray-800">
                Enter!
            </Button>
        </Modal>
